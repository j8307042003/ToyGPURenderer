...
static void SampleDisneyBsdf(const SurfaceData & surface, const DisneyBRDFParam & param, const glm::vec3 & wo, const glm::vec3 & wi, BsdfSample & bsdfSample)
{
	...
	auto specular = d * g * F / (4.0f * dotNL * dotNV);

	/*
	auto g = GeometrySmith(dotNV, dotNL, param.roughness);
	auto d = GTR2Aniso(wm.y, wm.x, wm.z, ax, ay);
	float G1 = SmithGAniso(dotNV, wo.x, wo.z, ax, ay);
	float G2 = G1 * SmithGAniso(dotNL, wi.x, wi.z, ax, ay);
	...
	*/
	...
	bsdfSample.reflectance =  diffuse * (1.0f - param.metallic) + specular;// *pdf;// specular;
}
