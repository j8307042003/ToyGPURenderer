...
CameraData CanonAnamorphic_camera_data::MakeCamData()
{
	std::shared_ptr<CanonAnamorphicCamData> data = std::make_shared<CanonAnamorphicCamData>();
	data->film = 0.036f;
	data->aperture = 1.0f;
	data->dist = 0.0f;
	CameraData camdata = {};
	
	camdata.dataType = std::type_index(typeid(CanonAnamorphicCamData));
	...
	camdata.sampleRay = [=](const vec3& pos, const vec3& direction, const vec2& filmRes, const vec2& pixelPos, vec3& transmittance, bool staticRay) {
			return CanonAnamorphic_camera_data::SampleCamRay(*data, pos, direction, filmRes, pixelPos, transmittance, staticRay);
	};
	
	return camdata;
}

Ray3f CanonAnamorphic_camera_data::SampleCamRay(const CanonAnamorphicCamData& cam, const vec3 & pos, const vec3 & direction, const vec2 & filmRes, const vec2 & pixelPos, vec3& transmittance, bool staticRay)
{
